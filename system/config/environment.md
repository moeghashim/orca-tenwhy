...
- Tests in tengrids use `vitest-canvas-mock`; our dashboard tests are plain `node --test` + jsdom (no canvas, no JSX loader) → grid *model* is unit-tested; mounting is smoke-tested with a stubbed `HTMLCanvasElement.prototype.getContext` + `ResizeObserver`. `ResearchGrid.js` uses `createElement` (no JSX). `make vendor-tengrids` removes the submodule's nested `react` / `react-dom` so Node resolves the root copies.
