...
- `make verify` / `make verify-fast` depend on `vendor-tengrids` and the dashboard `vite build`. Tengrids `build.sh` needs `/opt/homebrew/bin/bash` (5.3) on PATH ahead of macOS 3.2.
